...
void *forca_bruta(void *arg)
{
    ...
    unsigned long long contador_local = 0; // Contador local para a thread
    ...
    for (unsigned long long i = id_thread; i < total_combinacoes && !achou; i += qtd_threads)
    {
        ...
        if (contador_local % 1000 == 0)
        {
            atomic_fetch_add(&tentativas, 1000);
        }
        ...
        if (memcmp(tentativa, senha_alvo, tam_senha) == 0)
        {
            ...
            {
                printf("Thread %d encontrou a senha: %s\n", id_thread, tentativa);
    ...
    if (contador_local % 1000 != 0)
    {
        atomic_fetch_add(&tentativas, contador_local);
    }
